...
```{r aff_location_germany_contour_map,fig.cap="The location of academic organizations in Germany."}
aff_DEU = aff %>% filter(country == "Germany")
ggplot()+
  geom_hex(data = aff_DEU, aes(x = Longitude, y = Latitude),binwidth = c(1, 0.6),alpha = 0.5) +
...
p = ggplot() +
  ...
  geom_point(data = subset(aff,!is.na(Longitude) & !is.na(Latitude)),
                 aes(Longitude, Latitude),size = 0.1,color = prettyRed) +
	coord_equal() +
	  theme_void()
p
```
